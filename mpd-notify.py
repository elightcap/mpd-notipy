...
def get_album_art(title,artist,album,file):
    ffm = system("ffmpeg -i '{dir}/{filename}' {coverfile} -y &> /dev/null".format(dir=MUSICDIR,filename=file,coverfile=TEMPCOVER))
    if ffm == 256:
        print("not found")
        ...
        if get_image.status_code == 200:
            ...
            shutil.copyfile(PLACEHOLDER,TEMPCOVER)
...
def send_notification(song_note):
    system("dunstify -a Music --replace=27072 -t 2000 -i {albumcover} '{songinfo}'".format(albumcover=TEMPCOVER, songinfo=song_note))
